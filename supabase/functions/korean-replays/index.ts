...
serve(async (req) => {
  ...
  try {
    ...
    // Get API key from environment (check multiple possible secret names)
    let riotApiKey = Deno.env.get('RIOT_API_KEY') || Deno.env.get('RGAPI-40e54975-f7bd-4754-a207-c8c80136be31');
    
    console.log('Available environment variables:', Object.keys(Deno.env.toObject()));
    console.log('Riot API key found:', riotApiKey ? 'Yes' : 'No');
      ...
      try {
        ...
        for (const matchId of matchIds.slice(0, 2)) { // Limit to 2 matches per player
          try {
            ...
            // Apply filters with better logging and more lenient defaults
            console.log(`Checking match ${matchId} - Duration: ${Math.floor(match.info.gameDuration / 60)} mins, KDA: ${(participant.kills + participant.assists) / Math.max(participant.deaths, 1)}`);
            
            // Only apply tier filter if it's not Challenger (since we're only fetching Challenger)
            if (filters.tier && filters.tier !== 'Challenger' && filters.tier !== 'challenger') {
              console.log(`Skipping match due to tier filter: ${filters.tier}`);
              continue;
            }
            
            // More lenient duration filter - only apply if specified and reasonable
            const gameDurationMinutes = Math.floor(match.info.gameDuration / 60);
            if (filters.duration && filters.duration > 15 && gameDurationMinutes < filters.duration) {
              console.log(`Skipping match due to duration: ${gameDurationMinutes} < ${filters.duration}`);
              continue;
            }
            
            // More lenient KDA filter - only apply if specified and reasonable
            const kda = (participant.kills + participant.assists) / Math.max(participant.deaths, 1);
            if (filters.kda && parseFloat(filters.kda) > 1.0 && kda < parseFloat(filters.kda)) {
              console.log(`Skipping match due to KDA: ${kda} < ${filters.kda}`);
              continue;
            }
            
            console.log(`Match ${matchId} passed all filters!`);
    ...
    // Safely get filters, defaulting to empty object if not available
    let safeFilters = {};
    try {
      if (req.body) {
      ...
      console.log('Could not parse request body for filters');
    }
    
    return new Response(
      JSON.stringify({ 
        error: error.message,
        replays: generateMockKoreanReplays(safeFilters)
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200
