...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const toggleSystem = async () => {
    const newRunning = !systemStatus.isRunning;
    
    setSystemStatus(prev => ({
      ...prev,
      isRunning: newRunning,
      currentTask: newRunning ? "Starting automation..." : "Idle",
      lastActivity: "Just now"
    }));

    // Start automation when system starts
    if (newRunning) {
      toast({
        title: "System Starting",
        description: "Initializing League of Legends automation...",
        duration: 3000
      });

      // Update to show we're launching applications
      setSystemStatus(prev => ({
        ...prev,
        currentTask: "Launching League Client and OBS Studio..."
      }));

      try {
        // For web-based app, show instructions to user
        toast({
          title: "Manual Setup Required",
          description: "Please open League of Legends and OBS Studio manually, then start recording replays",
          duration: 8000
        });

        // Update task to show we're ready
        setTimeout(() => {
          setSystemStatus(prev => ({
            ...prev,
            currentTask: "Ready for recording - waiting for manual setup..."
          }));
        }, 3000);
        ...
        if (pendingJobs.length > 0) {
          setTimeout(() => {
            setSystemStatus(prev => ({
              ...prev,
              currentTask: "Processing upload queue..."
            }));
            ...
            setTimeout(() => updateJobStatus(pendingJobs[0].id, "processing", 50), 2000);
            setTimeout(() => updateJobStatus(pendingJobs[0].id, "uploading", 75), 4000);
            setTimeout(() => {
              updateJobStatus(pendingJobs[0].id, "completed", 100);
              setSystemStatus(prev => ({
                ...prev,
                currentTask: "Monitoring for new replays..."
              }));
            }, 6000);
          }, 5000);
        } else {
        ...
        toast({
          ...
          description: "Failed to start automation. Please set up manually.",
          variant: "destructive"
        });
        ...
        setSystemStatus(prev => ({
          ...prev,
          isRunning: false,
          currentTask: "Error - manual setup required"
        }));
      }
    } else {
      toast({
        title: "System Stopped",
        description: "Automation stopped. Close League Client and OBS when done.",
        duration: 3000
      });
    }
  };
