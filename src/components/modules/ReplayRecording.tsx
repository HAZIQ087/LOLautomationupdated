...
export const ReplayRecording = () => {
  ...
  const { replays } = useApp();
  
  // Use real replays data instead of mock data
  const [recordingJobs, setRecordingJobs] = useState<RecordingJob[]>(
    replays.slice(0, 3).map((replay, index) => ({
      id: replay.id,
      replay: {
        player: replay.player || "Unknown Player",
        champion: replay.champion,
        kda: replay.kda || "0/0/0"
      },
      status: index === 0 ? "recording" : "completed",
      progress: index === 0 ? 45 : 100,
      currentStep: index === 0 ? "Recording game replay with automation scripts" : "Recording completed",
      duration: `${replay.duration || 30}:00`,
      estimatedTime: index === 0 ? "18 min remaining" : "Completed"
    }))
  );
